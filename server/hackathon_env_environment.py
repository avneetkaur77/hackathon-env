...
class HackathonEnvironment:
    ...
    def step(self, action: HackathonAction):
        ...
        if step == 1:
            ...
            if "sorry" in response or "understand" in response: score += 0.2
            if str(gt["days"]) in response: score += 0.2
            if gt["is_urgent"] and "priority" in response: score += 0.3
            reward = min(1.0, score)

        done = step >= self.max_steps
        return HackathonObservation(
            ticket_text="Processing..." if not done else "Complete",
            reward=round(reward, 2),
            done=done,
            metadata={"step": step, "task_id": gt["id"], "days": gt["days"]}
        )
