...
def init_client():
    ...
    try:
        ...
        # ✅ Wrapped in try/except to prevent unhandled crash
        try:
            client = OpenAI(base_url=API_BASE_URL, api_key=API_KEY)
            # ✅ Top-level ping so validator sees proxy call
...
def intelligent_agent(obs):
    ...
    try:
        if client:
            ...
            print("[WARNING] Client not initialized, using fallback", flush=True)
    except Exception as e:
